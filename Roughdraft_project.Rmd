...
```{r, results='asis'}
#Calculate county-level measures (row)
# co_totals <- rowSums(race_clean)

#co_black <- sum(race_clean$Black)
#co_hisp <- sum(race_clean$Hispanic)
#co_white <- sum(race_clean$White)
...
# matrix_crime_race <- data.matrix(crime_race, rownames.force = NA)

blackwhite_seg <- spseg(sp_blackwhite, crime_race)
  
  dissim(data = sp_blackwhite)
